...
export default function App() {
  ...
  const saveNote = (dateKey, noteText) => {
    const newTrackerData = { 
      ...trackerData, 
      [dateKey]: { ...(trackerData[dateKey] || {}), note: noteText } 
    };
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900 pb-20 select-none overflow-x-hidden">
      <div className="max-w-7xl mx-auto px-2 md:px-4 pt-8 flex flex-col min-h-screen">
        <div className="flex-grow">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
            ...
            <div className="bg-white p-6 rounded-[2.5rem] border border-slate-200 shadow-sm relative overflow-hidden flex flex-col justify-between group">
              <div className="flex items-center justify-between mb-4"><p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Momentum</p><ActivityIcon /></div>
          ...
          {/* Main Log Table */}
          <div className="bg-white rounded-[2.5rem] shadow-xl border border-slate-200 overflow-hidden mb-8 relative">
              <div ref={scrollContainerRef} className="overflow-x-auto max-h-[70vh] overflow-y-auto custom-scrollbar scroll-smooth">
                  <table className="w-full border-separate border-spacing-0">
                      <thead className="sticky top-0 z-30 shadow-sm bg-slate-50 border-b border-slate-200">
                          <tr>
                            <th className="p-4 font-black text-slate-400 text-[9px] uppercase tracking-widest sticky top-0 left-0 bg-slate-50 border-r w-[130px] z-40 text-left">Date Log</th>
                            {habits.map((h, i) => (<th key={i} className="p-2 border-r border-slate-100 text-[9px] uppercase text-center font-bold text-slate-600 min-w-[90px] sticky top-0 bg-slate-50 z-30">{h}</th>))}
                            <th className="p-4 font-black text-emerald-600 text-[9px] sticky top-0 right-0 bg-slate-50 border-l w-[100px] z-40 text-center pr-4">Efficiency</th>
                          </tr>
                      </thead>
                      <tbody>
                          {daysInMonth.map((day) => {
        ...
        {/* Footer Credit Line */}
        <footer className="mt-8 mb-12 text-center border-t border-slate-200 pt-8">
            <p className="text-sm font-bold text-slate-400 tracking-wide">
              Developed by <a href="https://www.facebook.com/hsnshahriaradib" target="_blank" rel="noopener noreferrer" className="text-slate-600 font-black hover:text-emerald-600 transition-colors underline decoration-dotted underline-offset-4">Adib</a> | APM | RU
            </p>
        </footer>
      </div>
        ...
        <div className="fixed inset-0 z-[110] flex items-center justify-center bg-slate-900/60 backdrop-blur-md p-4" onClick={() => setShowAllNotes(false)}>
            <div className="bg-white rounded-[2.5rem] w-full max-w-2xl max-h-[80vh] overflow-hidden shadow-2xl animate-in zoom-in duration-200 flex flex-col" onClick={e => e.stopPropagation()}>
                <div className="p-8 border-b border-slate-100 flex items-center justify-between">
                    <div><h3 className="text-2xl font-black text-slate-800">Journal Archive</h3><p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">{currentDate.toLocaleString('default', { month: 'long', year: 'numeric' })} • {analytics.noteCount} Reflections</p></div>
                ...
                <div className="flex-1 overflow-y-auto p-8 space-y-6">
                    {currentMonthNotes.length > 0 ? currentMonthNotes.map((entry, idx) => (
                        <div key={idx} className="group border-l-4 border-emerald-500 pl-6 py-2 transition-all hover:bg-slate-50 rounded-r-2xl">
                            <div className="flex items-center gap-3 mb-2"><span className="text-[10px] font-black text-emerald-600 uppercase bg-emerald-50 px-2 py-1 rounded-md">{entry.date.toLocaleDateString(undefined, { day: 'numeric', month: 'short', weekday: 'short' })}</span><button onClick={() => { setShowAllNotes(false); setEditingNoteDate(entry.key); }} className="opacity-0 group-hover:opacity-100 text-slate-300 hover:text-blue-500 transition-all"><EditIcon /></button></div>
                            <p className="text-slate-600 font-medium leading-relaxed italic">"{entry.note}"</p>
                        </div>
                    )) : <div className="h-40 flex flex-col items-center justify-center text-slate-300 gap-3"><NoteIcon /><p className="font-bold text-sm">No reflections recorded yet.</p></div>}
                </div>
            </div>
        </div>
      )}
